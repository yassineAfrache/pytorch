...
void FuseListUnpack(
  ...
  for (auto node = graph_it.next(); node != nullptr; node = graph_it.next()) {
    ...
    replacement.list_unpack_node = list_unpack_node;
    replacement_list.emplace_back(replacement);
  }
  ...
  for (const auto& replacement : replacement_list) {
    ...
    for (Value* out : replacement.list_unpack_node->outputs()) {
    ...
    replacement.list_unpack_node->destroy();
