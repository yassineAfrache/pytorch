    ...
    class CudaGraphTreeTests(TestCase):
        ...
        def test_static_inputs_address_mutation_log(self):
            ...
            with self.assertRaisesRegex(
                ...
                r"input name: primals_2. data pointer changed from .* to .*\n"
                r"input name: primals_3. data pointer changed from .* to .*\n"
                r"related graph:(.*\n)*"
                r"related stack traces:.*, in forward.* return self.static_tensor"
                r" \+ x \+ self.goo\(x\).*\n",
            ):
                self.curr_node().run(
